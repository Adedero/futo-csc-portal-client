...
const items = ref([
    ...
    },
    {
        ...
        route: { name: 'dean-results' }
    },
    {
        label: 'Account',
        icon: 'pi pi-user',
        route: { name: 'dean-account' }
    },
    {
        label: 'Logout',
        icon: 'pi pi-sign-out',
...
async function getDeanNameAndEmail() {
    try {
        const response = await fetch(`${ import.meta.env.VITE_API }/dean/dean-header`, {
            credentials: 'include'
        })
        ...
        <template #app-header>
            <div class="flex items-center justify-end gap-2">
                <div class="hidden md:block text-right">
                    <p class="text-xs font-bold">{{ dean.name }}</p>
                    <p class="text-[0.65rem]">{{ dean.email ?? '' }}</p>
                </div>
                <div>
                    <img v-if="dean.imageURL" class="w-9 h-9 object-cover rounded-md" :src="dean.imageURL"
                        alt="user-image">
