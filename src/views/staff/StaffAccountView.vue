...
async function getStaffAccount() {
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/staff/account-details`, { credentials: 'include' });
...
async function uploadPicture() {
    ...
    try {
        ...
        // Send URL to backend
        const res = await fetch(`${import.meta.env.VITE_API}/staff/upload-picture`, {
            method: 'PUT',
            credentials: 'include',
            headers: {
...
async function changeUsername() {
    ...
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/staff/change-username?username=${account.value.username}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function updateProfile() {
    ...
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/staff/update-profile?staffId=${account.value.staff._id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function changePassword() {
    ...
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/staff/change-password`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
                            ...
                            <Fieldset>
                                ...
                                <div class="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                                    <div>
                                        ...
                                        <InputText :value="account.staff.name" readonly class="w-full" />
                                    </div>

                                    <div>
                                        <label class="text-sm font-semibold">Staff ID</label>
                                        <InputText :value="account.staff.staffId" readonly class="w-full" />

                                    </div>

                                    <div>
                                        <label class="text-sm font-semibold">Status</label>
                                        <InputText :value="account.staff.isAdvisor ? 'Advisor' : 'Regular'" readonly
                                            class="w-full" />
                                    </div>
