...
async function getResultForOGR() {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_API}/staff/result-ogr?staffId=${staffId}&session=${session}&semester=${semester}&level=${level}&course=${course}`, {
            credentials: 'include'
        })
        ...
        if (response.status == 200) {
            ...
            router.push({ name: 'staff-add-result' })
        }
    } catch (error) {
        console.error(error)
    }
...
function editResult(result) {
    router.push({ name: 'staff-edit-result', params: { resultId: result._id } });
}
</script>
