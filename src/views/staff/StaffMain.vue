...
const items = ref([
    ...
    },
    {
        ...
        route: { name: 'staff-results' }
    },
    {
        label: 'Account',
        icon: 'pi pi-user',
        route: { name: 'staff-account' }
    },
    {
        label: 'Logout',
        icon: 'pi pi-sign-out',
...
async function getStaffNameAndEmail() {
    try {
        const response = await fetch(`${ import.meta.env.VITE_API }/staff/staff-header`, {
            credentials: 'include'
        })
...
onMounted(() => getStaffNameAndEmail())
        ...
        <template #app-header>
            <div class="flex items-center justify-end gap-2">
                <div class="hidden md:block text-right">
                    <p class="text-xs font-bold">{{ staff.name }}</p>
                    <p class="text-[0.65rem]">{{ staff.email ?? '' }}</p>
                </div>
                <div>
                    <img v-if="staff.imageURL" class="w-9 h-9 object-cover rounded-md" :src="staff.imageURL"
                        alt="user-image">
