...
async function getCourseRegDetails() {
    ...
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/advisor/student-course-registration-details?studentId=${studentId}&session=${session}`, {
            credentials: 'include'
        })
    ...
    <main class="w-full h-full overflow-y-scroll p-5">
        <Card class="overflow-scroll">
            ...
            <template #content>
                ...
                <div style="height: calc(100dvh - 14rem)" class="record-container overflow-scroll mt-4">

                    <div v-for="record in details" :key="record._id"
                        class="reg-courses-details overflow-y-scroll bg-white text-slate-700 rounded p-2 pb-10 md:gap-2 w-[210mm] h-[320mm] border-2">
                        ...
                        <div class="gap-2 mt-4 w-full grid grid-cols-5 p-5">
                            <img v-if="record.student.imageURL" :src="record.student.imageURL" alt="user"
                            ...
                            <div class="text-[0.8rem] w-full items-center gap-2 p grid grid-cols-2 col-span-4">
                                ...
                                <div class="grid grid-cols-3 gap-2 w-full">
                                    ...
                                    <span class="col-span-2 uppercase font-semibold">{{ record.student.name }}</span>
                                </div>
                                ...
                                <div class="grid grid-cols-3 gap-2 w-full">
                                    ...
                                    <span class="col-span-2 uppercase font-semibold">
                                        {{ record.student.entryMode ?? '' }}
                                    </span>
                                </div>

                                <div class="grid grid-cols-2 gap-2 w-full">
                                    <p class="text-left">Level:</p>
                                    <span class="uppercase font-semibold">{{ record.level }}</span>
                                </div>
