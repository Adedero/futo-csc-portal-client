...
async function getResultForOGR() {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_API}/advisor/result-ogr?staffId=${staffId}&session=${session}&semester=${semester}&level=${level}&course=${course}`, {
            credentials: 'include'
        })
        ...
        if (response.status == 200) {
            ...
            router.push({ name: 'advisor-add-result' })
        }
    } catch (error) {
        console.error(error)
    }
        ...
        <Card class="overflow-scroll">
            <template #title>
                <div class="flex gap-3 justify-between item-center max-w-[210mm]">
                    ...
                    <div class="flex gap-4">
                        <!-- Edit button should be disabled if approved by HOD or approved by Dean or if the staffId is not equal to the id of the advisor -->
                        <Button @click="editResult(result)" label="Edit"
                            :disabled="result.isApprovedByHOD || result.isApprovedByDean || result.staffId !== route.params.staffId" />
            ...
            <template #content>
                ...
                <!-- <div style="height: calc(100dvh - 14rem)" id="ogr" class="print-content overflow-scroll mt-4">
                    ...
                    <div
                    v-for="(group, index) in paginatedStudents" :key="index"
                    class="result-ogr overflow-y-scroll bg-white text-slate-700 rounded p-5 pb-10 md:gap-2 w-[210mm] h-[297mm] border-2">
                        ...
                        <div class="flex items-center gap-16 text-[0.63rem]">
                            ...
                            <div>
                                ...
                                <p>Date: {{ new Date(result.updatedAt).toDateString() }}</p>
                            </div>
                        </div>
