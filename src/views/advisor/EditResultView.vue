...
async function getExistingResult() {
    try {
        const response = await fetch(`${import.meta.env.VITE_API}/advisor/result-to-edit?resultId=${route.params.resultId}`, {
            credentials: 'include'
        })
...
async function saveEdit() {
    loading.value = true
    try {
        const response = await fetch(`${import.meta.env.VITE_API}/advisor/save-result-edit`, {
            method: 'PUT',
            headers: {
