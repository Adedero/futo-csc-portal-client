...
async function getResultForOGR() {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_API}/admin/result-ogr?staffId=${staffId}&session=${session}&semester=${semester}&level=${level}&course=${course}`, {
            credentials: 'include'
        })
        ...
        <Card class="overflow-scroll">
            <template #title>
                ...
                <div class="text-sm">
                    ...
                    <p :class="[result.isApprovedByHOD ? 'text-green-500' : 'text-red-500', 'text-xs italic']">
                        {{ result.isApprovedByHOD ? 'Approved by H.O.D' : 'Awaiting H.O.D approval' }}
                    </p>
                    <p :class="[result.isApprovedByDean ? 'text-green-500' : 'text-red-500', 'text-xs italic']">
                        {{ result.isApprovedByDean ? 'Approved by Dean' : 'Awaiting Dean approval' }}
                    </p>
                </div>
            </template>
