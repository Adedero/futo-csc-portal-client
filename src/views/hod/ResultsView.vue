...
async function getResults() {
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/hod/pending-results`, {
            credentials: 'include'
        })
...
const approvedResults = computed(() => {
    return results.value.filter(result => result.isApprovedByHOD || result.isApprovedByDean)
})
...
async function approveResult(result) {
    result.loading = true
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/hod/approve-result/${result._id}`, {
            method: 'PUT',
            credentials: 'include'
        })
        const data = await res.json()
        ...
        if (res.status === 200) {
            ...
            toast.add({ severity: 'success', summary: 'Approved', detail: data.message, life: 3000 })
        }
    } catch (error) {
        console.error(error)
    } finally {
...
async function disapproveResult(result) {
    ...
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/hod/disapprove-result/${result._id}`, {
            method: 'PUT',
            credentials: 'include'
        })
        const data = await res.json()
        ...
        if (res.status === 200) {
            ...
            toast.add({ severity: 'error', summary: data.info, detail: data.message })
        }
    } catch (error) {
        console.error(error)
    } finally {
                                        ...
                                        <tr v-for="result in filteredApprovedResults" :key="result._id">
                                            ...
                                            <td class="flex items-center justify-between gap-2">
                                                ...
                                                <Button @click="disapproveResult(result)" severity="secondary"
                                                    label="Disapprove" icon="pi pi-times"
                                                    :loading="disapproveResultLoading" />
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
