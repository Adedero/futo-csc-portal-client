...
const items = ref([
    ...
    },
    {
        ...
        route: { name: 'hod-staff' }
    },
    {
        label: 'Results',
        icon: 'pi pi-chart-line',
        route: { name: 'hod-results' }
    },
    {
        label: 'Course Allocation',
        icon: 'pi pi-book',
        route: { name: 'hod-course-allocation' }
    },
    {
        label: 'Applications',
        icon: 'pi pi-file',
...
async function getHODNameAndEmail() {
    try {
        const response = await fetch(`${ import.meta.env.VITE_API }/hod/hod-header`, {
            credentials: 'include'
        })
        ...
        <template #app-header>
            <div class="flex items-center justify-end gap-2">
                <div class="hidden md:block text-right">
                    <p class="text-xs font-bold">{{ HOD.name }}</p>
                    <p class="text-[0.65rem]">{{ HOD.email ?? '' }}</p>
                </div>
                <div>
                    <img v-if="HOD.imageURL" class="w-9 h-9 object-cover rounded-md" :src="HOD.imageURL"
                        alt="user-image">
