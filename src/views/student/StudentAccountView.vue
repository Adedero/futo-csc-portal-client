...
let originalDetails = reactive({
    dateOfBirth: account.value.student.dateOfBirth,
    sex: account.value.student.sex,
    stateOfOrigin: account.value.student.stateOfOrigin,
    nationality: account.value.student.nationality,
    phoneNumber: account.value.student.phoneNumber,
    email: account.value.student.email
})

let isDisabled = computed(() => {
    for (let key of Object.keys(originalDetails)) {
        if (originalDetails[key] !== account.value.student[key]) {
            return false;
        }
    }
...
function onSelect(event) {
    ...
    messageText.value = 'You cannot change your profile picture once uploaded.'
}
...
async function uploadPicture() {
    ...
    try {
        ...
        if (account.value.student.imageURL) {
            await deleteFile(account.value.student.imageURL);
        ...
        // Send URL to backend
        const res = await fetch(`${import.meta.env.VITE_API}/student/upload-picture`, {
            method: 'PUT',
            credentials: 'include',
            headers: {
            ...
            body: JSON.stringify({
                studentId: account.value.student._id,
                url: url
            })
        });
...
async function updateProfile() {
    ...
    if (account.value.student.dateOfBirth) {
        account.value.student.dateOfBirth = useDate(account.value.student.dateOfBirth)
    }

    let profile = {}
    for (let key of Object.keys(originalDetails)) {
        profile[key] = account.value.student[key]
    }
    ...
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/student/update-profile?studentId=${account.value.student._id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (res.status === 200) {
            ...
            for (let key of Object.keys(profile)) {
                account.value.student[key] = profile[key]
...
async function changePassword() {
    ...
    try {
        const res = await fetch(`${import.meta.env.VITE_API}/student/change-password`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
<template>
    <EmailAlert :user="account.student" />
                        ...
                        <div class="lg:col-span-2">
                            <Fieldset>
                                ...
                                <div class="flex flex-col items-center justify-center">
                                    <div v-if="tempUrl || account.student.imageURL">
                                        <img v-if="tempUrl" :src="tempUrl" alt="user"
                                            class="w-24 h-24 rounded-full object-cover">
                                        <img v-else :src="account.student.imageURL" alt="user"
                                            class="w-24 h-24 rounded-full object-cover">
                                    </div>
                                    ...
                                    <div class="mt-2">
                                        <label v-if="!account.student.imageURL" ref="label" for="profile-picture"
                                    ...
                                    <Message v-if="!account.student.imageURL" icon="pi pi-info-circle"
                                        :closable="false">
                                        <p class="text-xs">
                                            {{ messageText }}
                                        </p>
