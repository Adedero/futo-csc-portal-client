...
const items = ref([
    ...
    },
    {
        label: 'Courses',
        icon: 'pi pi-book',
        route: { name: 'registered-courses' }
    },
    {
        label: 'Results',
        icon: 'pi pi-chart-line',
        route: { name: 'student-results' }
    },
    {
        label: 'Account',
        icon: 'pi pi-user',
        route: { name: 'student-account' }
    },
    {
        label: 'Logout',
        icon: 'pi pi-sign-out',
...
async function getStudentNameAndEmail() {
    try {
        const response = await fetch(`${ import.meta.env.VITE_API }/student/student-header`, {
            credentials: 'include'
        })
...
onMounted(() => getStudentNameAndEmail())
        ...
        <template #app-header>
            <div class="flex items-center justify-end gap-2">
                <div class="hidden md:block text-right">
                    ...
                    <p class="text-[0.65rem]">{{ student.email ?? '' }}</p>
                </div>
                <div>
                    <img v-if="!student.imageURL" class="w-9 h-9 object-cover rounded-md"
                        src="../../assets/images/user.jpg" alt="user-image">
                    <img v-else class="w-9 h-9 object-cover rounded-md" :src="student.imageURL" alt="user-image">
                </div>
            </div>
        </template>
